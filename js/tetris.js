...
Pieza.prototype.girarDerecha = function(){
    if(this.contacto == 0 && this.posX + this.ancho < 319){
        var url;
        switch(this.tipo){
            case 0:
                if(this.inc == 1){
...
Pieza.prototype.caer = function(){
    ...
    if(comprobarCasillasInferiores(p) || this.posY + this.alto >= 600){
        this.contacto = 1;
...
function moverPiezaD(){
    if(!comprobarDerecha(p) && p.contacto == 0 && p.posX + p.ancho < 319){ // si la pieza no ha caído, y no toca los bordes
        blanquear2(p.posX, p.posY, p.ancho, p.alto, p.tipo, p.inc); 
        p.posX += 20;
        p.cargarImg();
        p.imagen.onload = function(){
...
function copiarCanvas(y){
    var imagen = context.getImageData(0, 0, 320, y); // Copiamos el canvas hasta la línea a borrar
    ...
    switch(pieza.alto){
        case 20:
            ...
            if(a == 16){
                ...
                numLineas++;
                contNivel++;
                document.getElementById("marcador").innerHTML = ("LÍNEAS: " + numLineas);
            }
            break;
        case 40:
            for(var i = y; i < y + 2; i++){
